...
import { supabase, type Bodyguard, type Client } from '../lib/supabase';
...
const DashboardPage: React.FC = () => {
  ...
  // Handle booking
  const handleBooking = async (bodyguardId: string): Promise<{ success: boolean; message: string }> => {
    if (!user) {
    ...
    try {
      ...
      if (!bodyguard) {
        return { success: false, message: 'Bodyguard not found.' };
      }

      // Create booking with default 1-hour duration
        ...
        .insert([
          {
            ...
            duration_hours: 1,
            total_amount: bodyguard.hourly_rate,
            status: 'pending'
          }
        ]);
